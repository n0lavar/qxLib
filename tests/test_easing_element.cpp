...
protected:
        ...
        type fExpectedSpeed = type(1.f))
    {
        ...
        EXPECT_TRUE(qx::epsilon_equal(
            m_Element.get_fraction(),
            fExpectedFraction,
            eps));
        EXPECT_TRUE(
            qx::epsilon_equal(m_Element.get_speed(), fExpectedSpeed, eps));
    }
...
TYPED_TEST(TestEasingElements, update_speed)
{
    ...
    EXPECT_TRUE(
        qx::epsilon_equal(TestFixture::m_Element.update(TYPE(0.1)), TYPE(0.f)));
    ...
    EXPECT_TRUE(
        qx::epsilon_equal(TestFixture::m_Element.update(TYPE(0.2)), TYPE(0.f)));
    ...
    EXPECT_TRUE(qx::epsilon_equal(
        TestFixture::m_Element.update(TYPE(1.0)),
        TYPE(0.9f),
        TYPE(1e-7)));
    TestFixture::CheckFinished(TYPE(2.l));

    EXPECT_TRUE(
        qx::epsilon_equal(TestFixture::m_Element.update(TYPE(1.0)), TYPE(1.f)));
    TestFixture::CheckFinished(TYPE(2.l));
}
...
TYPED_TEST(TestEasingElements, pause_resume)
{
    ...
    EXPECT_TRUE(qx::epsilon_equal(
        TestFixture::m_Element.update(TYPE(0.1f)),
        TYPE(0.1f)));
    TestFixture::CheckPaused(TYPE(-0.7l), TYPE(0.1l));
