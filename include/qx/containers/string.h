...
public:
    ...
    int compare(const_pointer pszStr, size_type nStrSize) const noexcept;
    ...
    vector split(const_pointer pszSeparator, size_type nSepLen = npos)
        const noexcept;
    vector split(const basic_string& sSeparator) const noexcept;
    template<class FwdIt>
    vector split(FwdIt itSepFirst, FwdIt itSepLast) const noexcept;
    template<string_convertable String>
...
#define _QX_STR_OP_PLUS_BODY                  \
    {                                         \
        basic_string<UT> str(std::move(lhs)); \
        str += rhs;                           \
        return std::move(str);                \
    }

template<class UT>
basic_string<UT> operator+(
    const basic_string<UT>& lhs,
    const basic_string<UT>& rhs) noexcept _QX_STR_OP_PLUS_BODY
    template<class UT>
    basic_string<UT> operator+(
        basic_string<UT>&& lhs,
        basic_string<UT>&& rhs) noexcept _QX_STR_OP_PLUS_BODY template<class UT>
    basic_string<UT> operator+(
        basic_string<UT>&&      lhs,
        const basic_string<UT>& rhs) noexcept _QX_STR_OP_PLUS_BODY
    template<class UT>
    basic_string<UT> operator+(
        const basic_string<UT>& lhs,
        basic_string<UT>&&      rhs) noexcept _QX_STR_OP_PLUS_BODY

    template<class UT>
    basic_string<UT> operator+(
        const basic_string<UT>&    lhs,
        typename UT::const_pointer rhs) noexcept _QX_STR_OP_PLUS_BODY
    template<class UT>
    basic_string<UT> operator+(
        basic_string<UT>&&         lhs,
        typename UT::const_pointer rhs) noexcept _QX_STR_OP_PLUS_BODY
    ...
    basic_string<UT> operator+(
        const basic_string<UT>& lhs,
        typename UT::value_type rhs) noexcept _QX_STR_OP_PLUS_BODY
    template<class UT>
    basic_string<UT> operator+(
        basic_string<UT>&&      lhs,
        typename UT::value_type rhs) noexcept _QX_STR_OP_PLUS_BODY
    ...
    basic_string<UT> operator+(
        const basic_string<UT>& lhs,
        const String&           rhs) noexcept _QX_STR_OP_PLUS_BODY
    template<class UT, string_convertable String>
    basic_string<UT> operator+(
        basic_string<UT>&& lhs,
        const String&      rhs) noexcept _QX_STR_OP_PLUS_BODY
