...
inline T exponential_in_func(T x) noexcept
{
    return x <= T(0.0f)
        ? T(0.0f)
        : std::pow(T(2.0f), T(10.0f) * (x - T(1.0f)));
}
...
inline T exponential_out_func(T x) noexcept
{
    return x >= T(1.0f)
        ? T(1.0f)
        : T(1.0f) - std::pow(T(2.0f), -T(10.0f) * x);
}
...
{
    return T(1.0f) - std::pow(T(2.0f), -T(10.0f) * x)
...
inline T elastic_in_out_func(T x) noexcept
{
    ...
    {
        return T(1.0f) - T(0.5f) * std::pow(T(2.0f), T(10.0f) * (T(1.0f) - T(2.0f) * x))
            * static_cast<T>(std::sin(T(13.0f) * std::numbers::pi_v<T> * x));
    }
}
...
inline T back_in_out_func(T x) noexcept
{
    if (x < T(0.5f))
    {
        ...
        return T(0.5f) * f2t
            * (f2t * f2t - static_cast<T>(std::sin(std::numbers::pi_v<T> * f2t)));
    }
    else
    {
        const T fInv = T(2.0f) - T(2.0f) * x;
        return T(1.0f) - T(0.5f) * fInv
            * (fInv * fInv - static_cast<T>(std::sin(std::numbers::pi_v<T> * fInv)));
    }
}
